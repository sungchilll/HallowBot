                ...
                if(turn !=0):
                    if(turn%20 == 0):
                        ...
                        if prediction < 0.5: # 마스크 미착용으로 판별되면, 
                            ...
                            return result
            turn +=1
        resize = cv2.resize(frame, dsize=(0, 0), fx=1.0, fy=0.9, interpolation=cv2.INTER_AREA)
        winname = "Mask Detecting"
        cv2.namedWindow(winname)
        cv2.moveWindow(winname, 700, 300)
        #cv2.imshow("mask nomask classify", resize)
